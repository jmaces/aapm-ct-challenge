...
train_params = {
    ...
    "optimizer_params": [{"lr": 5e-5, "eps": 1e-5, "weight_decay": 1e-4}],
...
iter_preinit = 4
subnet_list = []
for i in range(iter_preinit):
    subnet_tmp = subnet(**subnet_params).to(device)
    it_net_tmp = IterativeNet(
        iter_preinit * [subnet_tmp],
        **{
            "num_iter": iter_preinit,
            "lam": iter_preinit * [0.0],
            "lam_learnable": True,
            "final_dc": True,
            "resnet_factor": 1.0,
            "inverter": operator.inv,
    ...
    lam = it_net_tmp.lam
    subnet_list.append(it_net_tmp.subnet[i])

for i in range(iter_preinit, iter_max):
    subnet_tmp = subnet(**subnet_params).to(device)
    it_net_tmp = IterativeNet(
        iter_preinit * [subnet_tmp],
        **{
    ...
    subnet_list.append(it_net_tmp.subnet[-1])
...
it_net.lam[-2].data = torch.tensor(1.0, device=device)
